...
class Fusion_Template_Builder extends AWB_Layout_Conditions {
	...
	public function builder_check_singular_condition( $condition, $target_post ) {
		...
		if ( $sub_condition && false !== strpos( $singular_type, 'specific_' ) ) {
			...
			if ( ! $target_post ) {
				...
				if ( class_exists( 'WooCommerce' ) && fusion_is_shop( $c_page_id ) ) {
					$target_post = get_post( $c_page_id );
				} else {
					return $exclude;
				}
	...
	public function remove_third_party_the_content_changes( $override = false ) {
		...
		if ( class_exists( 'Tribe__Events__Main' ) ) {
			...
			if ( function_exists( 'tribe' ) && class_exists( 'TEC\Events_Pro\Custom_Tables\V1\Templates\Series_Filters' ) && $priority = has_filter( 'the_content', [ tribe( 'TEC\Events_Pro\Custom_Tables\V1\Templates\Series_Filters' ), 'inject_content' ] ) ) {
				remove_filter( 'the_content', [ tribe( 'TEC\Events_Pro\Custom_Tables\V1\Templates\Series_Filters' ), 'inject_content' ], $priority );

				if ( class_exists( 'TEC\Tickets\Flexible_Tickets\Series_Passes\Series_Passes' ) ) {
					remove_filter( 'the_content', [ tribe( 'TEC\Tickets\Flexible_Tickets\Series_Passes\Series_Passes' ), 'reorder_series_content' ], 0 );
				}
			}
		}
		...
		if ( 1 === $this->rendering_override_loop && class_exists( 'iHomefinderVirtualPageDispatcher' ) ) {
	...
	public function readd_third_party_the_content_changes( $override = false ) {
		...
		if ( class_exists( 'Tribe__Events__Main' ) ) {
			...
			if ( function_exists( 'tribe' ) && class_exists( 'TEC\Events_Pro\Custom_Tables\V1\Series\Post_Type' ) && $queried_object instanceof WP_Post && tribe( 'TEC\Events_Pro\Custom_Tables\V1\Series\Post_Type' )->is_same_type( $queried_object ) ) {
				...
				if ( class_exists( 'TEC\Tickets\Flexible_Tickets\Series_Passes\Series_Passes' ) ) {
					add_filter( 'the_content', [ tribe( 'TEC\Tickets\Flexible_Tickets\Series_Passes\Series_Passes' ), 'reorder_series_content' ], 0 );
				}
			}
		}
				...
				foreach ( $wp_filter['the_content'] as $index => $actions ) {
					foreach ( $actions as $name => $action ) {
						...
						if ( is_array( $action['function'] ) && isset( $action['function'][0] ) && is_object( $action['function'][0] ) ) {
							...
							if ( 'STM_LMS_Page_Router' === get_class( $action['function'][0] ) && false !== strpos( $name, 'change_page_content' ) ) {
								$this->handle_content_filter( $action['function'], $index );
							}
						}
					}
	...
	public function add_post_data( $data, $page_id, $post_type ) {
		...
		if ( $post ) {
			...
			$post_id       = $is_term ? $post->term_id . '-archive' : $post->ID;
			...
			$page_values = ! $is_term ? fusion_data()->post_meta( $post_id )->get_all_meta() : fusion_data()->term_meta( $post_id )->get_all_meta();

			if ( is_array( $page_values ) ) {
				$page_values['_thumbnail_id'] = get_post_thumbnail_id( $post_id );
			}

			$data['examplePostDetails'] = [
				'post_id'        => $post_id,
				'post_permalink' => get_permalink( $post ),
				'post_title'     => -99 === $post->ID ? apply_filters( 'the_title', $post->post_title, $post_id ) : get_the_title( $post_id ),
				'post_content'   => $content,
				'post_name'      => $post->post_name,
				'post_type'      => $post->post_type,
				'post_type_name' => is_object( $post_type_obj ) ? $post_type_obj->labels->singular_name : esc_html__( 'Page', 'fusion-builder' ),
				'post_status'    => -99 === $post_id ? $post->post_status : get_post_status( $post_id ),
	...
	public function dynamic_data( $post_data ) {
		if ( 'fusion_tb_section' === $post_data['post_type'] || fusion_is_post_card() || 'awb_off_canvas' === $post_data['post_type'] ) {
			...
			$post_data['id'] = $post instanceof WP_Term ? $post->term_id . '-archive' : $post->ID;

			if ( $post && ! $post instanceof WP_Term ) {
	...
	public function dynamic_id( $id ) {
		...
		if ( 'fusion_tb_section' === $post_type || fusion_is_post_card() || 'awb_off_canvas' === get_post_type( $id ) ) {
			...
			if ( $post ) {
				return $post instanceof WP_Term ? $post->term_id . '-archive' : $post->ID;
			}
		}
