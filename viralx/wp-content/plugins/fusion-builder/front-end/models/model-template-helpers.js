...
_.mixin( {
	...
	fusionGetBackgroundSliderElement: function( self, type ) {
		...
		let images = [];
		if ( 'object' === typeof values.background_slider_images ) {
			if ( 'undefined' !== typeof values.background_slider_images[ 0 ] && 'undefined' !== typeof values.background_slider_images[ 0 ].id ) {
				images = Object.values( values.background_slider_images ).map( image => image.id );
			} else {
				images = Object.values( values.background_slider_images );
			}
		} else {
			images = values.background_slider_images.split( ',' );
		}
		...
		if ( 'string' === typeof values.background_slider_images && values.background_slider_images.startsWith( 'http' ) ) {
	...
	fusionGetFontStyle: function( param_id, values, format = 'string', important = false ) {
		...
		if ( 'string' === typeof values[ 'fusion_font_family_' + param_id ] && '' !== values[ 'fusion_font_family_' + param_id ] ) {
			...
			} else if ( values[ 'fusion_font_family_' + param_id ].includes( '\'' ) || 'inherit' === values[ 'fusion_font_family_' + param_id ] || values[ 'fusion_font_family_' + param_id ].includes( ',' ) || values[ 'fusion_font_family_' + param_id ].includes( 'var(' ) ) {
				style[ 'font-family' ] = values[ 'fusion_font_family_' + param_id ];
			} else {
				style[ 'font-family' ] = '\'' + values[ 'fusion_font_family_' + param_id ] + '\'';
			}
	...
	awbResolveCSSVarToValue: function( value, element = document.documentElement ) {
