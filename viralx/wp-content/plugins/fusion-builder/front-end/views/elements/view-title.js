	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.fusion_title = FusionPageBuilder.ElementView.extend( {
			...
			buildHeadingAttr: function( values ) {
				...
				let globalTypo    = {};
				let fontSizeValue = values.font_size;
				if ( ! values.font_size ) {
					if ( jQuery.isNumeric( values.size ) ) {
						globalTypo = FusionApp.settings[ 'h' + values.size + '_typography' ];
					} else {
						globalTypo = FusionApp.settings.body_typography;
					}
				}

				fontSizeValue = 'undefined' !== typeof globalTypo[ 'font-size' ] ? globalTypo[ 'font-size' ] : fontSizeValue;
				fontSizeValue = _.awbResolveCSSVarToValue( fontSizeValue );
