...
( function() {
	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.BaseView = window.wp.Backbone.View.extend( {
			...
			/**
			 * Runs just after render or cancel.
			 *
			 * @since 3.0.2
			 * @return null
			 */
			...
			renderInlineSettings: function() {
				...
				if ( 'undefined' !== typeof newlyAdded ) {
					...
					const shortcode = newlyAdded.get( 'inlineElement' ); //newlyAdded.attributes.inlineElement;
					const newShortcode = shortcode.replace( ' open_settings="true"', '' );
					newlyAdded.set( 'inlineElement', newShortcode );

					newlyAdded.parentView.model.attributes.params.element_content = newlyAdded.parentView.model.attributes.params.element_content.replace( shortcode, newShortcode );
			...
			initDynamicParams: function() {
				...
				if ( 'string' === typeof params.dynamic_params && '' !== params.dynamic_params ) {
					try {
						if ( FusionPageBuilderApp.base64Encode( FusionPageBuilderApp.base64Decode( dynamicData ) ) === dynamicData ) {
							...
							parsedDynamicData = JSON.parse( dynamicData );
						}
					} catch ( error ) {
					...
					// Handle the inline dynamic data. Add to the main object as array, for differentiation from "block" dynamic data.
					const element          = fusionAllElements[ this.model.get( 'element_type' ) ],
						allowedOptionTypes = [ 'textfield', 'textarea', 'tinymce' ],
						ignoredOptions     = [ 'animation_', 'columnn_direction', 'margin_', 'padding_', 'max_', 'min_'];

					if ( ! _.isEmpty( params ) ) {
						if ( 'awb_dd' === element.shortcode ) {
							parsedDynamicData['awb_dd_output'] = [ this.buildParamsObject( params, false ) ];
						} else {
							_.each( params, function( value, name ) {
								if ( 'undefined' !== typeof value && '' !== value && 'undefined' !== typeof element.params[ name ] && allowedOptionTypes.includes( element.params[ name ].type ) && ! ignoredOptions.some( sub => value.toString().includes( sub ) ) ) {
									const inlineDynamicData = value.toString().match( /\{.*?\}/g );

									if ( ! _.isEmpty( inlineDynamicData ) ) {
										parsedDynamicData[ name ] = inlineDynamicData;
									}
								}
							} );
						}
				...
				// Stroe dynamic data.
				if ( !  _.isEmpty( parsedDynamicData ) ) {
					self.dynamicParams.setData( parsedDynamicData );
				}

				this.listenTo( FusionEvents, 'awb-options-change', this.updateDynamicAtts );
			},

			updateDynamicAtts: function( paramName, paramValue, cid ) {
				if ( cid !== this.model.get( 'cid' ) ) {
					return
				}

				const self             = this,
					element            = fusionAllElements[ self.model.get( 'element_type' ) ],
					allowedOptionTypes = [ 'textfield', 'textarea', 'tinymce' ],
					ignoredOptions     = [ 'animation_', 'columnn_direction', 'margin_', 'padding_', 'max_', 'min_'];

				let dynamicParams     = self.dynamicParams.getAll()

				if ( 'undefined' !== typeof paramValue && '' !== paramValue && 'undefined' !== typeof element.params[ paramName ] && allowedOptionTypes.includes( element.params[ paramName ].type ) && ! ignoredOptions.some( sub => paramValue.includes( sub ) ) ) {
					const inlineDynamicData = paramValue.match( /\{.*?\}/g );

					if ( ! _.isEmpty( inlineDynamicData ) ) {
						dynamicParams[ paramName ] = inlineDynamicData;
					} else {
						delete dynamicParams[ paramName ];
					}

					self.dynamicParams.setData( dynamicParams );
				}
			},
			...
			getDynamicAtts: function( atts ) {
				...
				if ( 'undefined' !== typeof self.dynamicParams && self.dynamicParams && ! _.isEmpty( self.dynamicParams.getAll() ) ) {
					_.each( self.dynamicParams.getAll(), function( data, id ) {

						// Handle "block" dynamic data.
						if ( ! Array.isArray( data ) ) {
							const value = self.dynamicParams.getParamValue( data );
							...
							// Handle inline dynamic data.
							let paramValue = '';

							// Dynamic data element.
							if ( 'awb_dd_output' === id ) {
								paramValue = FusionPageBuilderApp.dynamicValues.getValue( { 'data': data[0] } );

								if ( 'object' === typeof paramValue && 'function' === typeof paramValue.then ) {
									paramValue.then(
										function() {
											let params = self.model.get( 'params' );
											params[ id ] = paramValue.responseJSON.content;
											self.model.set( 'params', params );
											FusionPageBuilderApp.inlineEditorHelpers.processInlineElement( self.model, id );
										},
									);
								} else {
									atts.values[ id ] = paramValue;
								}
							} else if ( 'undefined' === typeof atts.values.disable_idd || 'no' === atts.values.disable_idd ) {
								paramValue = atts.values[ id ];

								_.each( data, function( endpoint, index ) {

									// The object with "data" as index is needed because of the "block" dynamic data structure.
									const value = FusionPageBuilderApp.dynamicValues.getValue( { 'data': endpoint } );

									if ( 'object' === typeof value && 'function' === typeof value.then ) {
										value.then(
											function() {
												if ( null !== value.responseJSON.content ) {
													const markup = '<span class="awb-dynamic-data-outline awb-dynamic-data"><span class="awb-dd-params">' + endpoint + '</span><span class="awb-dd-output">' + value.responseJSON.content + '</span></span>';
													paramValue = paramValue.replace( endpoint, markup );
													self.reRender();
												}
											},
										);
									} else {
										const markup  = '<span class="awb-dynamic-data-outline awb-dynamic-data"><span class="awb-dd-params">' + endpoint + '</span><span class="awb-dd-output">' + value + '</span></span>';
										paramValue = paramValue.replace( endpoint, markup );
									}
								} );
			...
			settings: function( event ) {
				...
				if ( 'dialog' === FusionApp.preferencesData.editing_mode || generated ) {
					...
					jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).addClass( 'fusion-sidebar-ui-active' );
			...
			onSettingsOpen: function() {
				jQuery( this.el ).addClass( 'awb-element-settings-open' );
			},
			...
			onSettingsClose: function() {
				...
				if ( 0 < $dialog.length && ! jQuery( 'body' ).hasClass( 'fusion-settings-dialog-large' ) ) {
					...
					jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).removeClass( 'fusion-sidebar-ui-active' );
				}

				jQuery( this.el ).removeClass( 'awb-element-settings-open' );
			},
