...
( function() {
	...
	FusionPageBuilder.AppView = Backbone.View.extend( {
		...
		initialize: function() {
			...
			this.listenTo( window.FusionEvents, 'fusion-preferences-rendering_logic-updated', this.toggleDeviceRenderingLogicPreview );
			this.listenTo( window.FusionEvents, 'fusion-preferences-inline_dynamic_data_preview-updated', this.toggleInlineDynamicDataPreview );
			...
			this.inlineElements = [ 'awb_dd', 'fusion_highlight', 'fusion_tooltip', 'fusion_dropcap', 'fusion_popover', 'fusion_one_page_text_link', 'fusion_modal_text_link' ];
		...
		contextMenu: function( event ) {
			...
			if ( ! inlineElement ) {
				...
				// This is an inline element, so we have to create the data and do checks.
		...
		generateElementShortcode: function( $element, openTagOnly, generator ) {
			...
			if ( 'params' in elementSettings ) {
				...
				for ( param in settingValue ) {
					...
					if ( 'element_content' === keyName ) {
						...
						if ( 'awb_dd' === elementType ) {
							if ( ( 'dynamic_data' !== keyName && 'open_settings' !== keyName && 'hide_on_mobile' !== keyName && 'class' !== keyName && 'id' !== keyName && ( -1 === keyName.indexOf( settingValue.dynamic_data ) || '' === settingValue.dynamic_data ) ) || ( ( 'hide_on_mobile' === keyName || 'class' === keyName || 'id' === keyName ) && '' === settingValue[ keyName ] ) ) {
								ignored = keyName;
							}
						}
            ...
            if ( ('' === content && 'fusion_text' !== elementType && 'fusion_code' !== elementType && ( 'undefined' !== typeof elementSettings.type && 'element' === elementSettings.type ) || 'awb_dd' === elementType ) ) {
                openTagOnly = true;
                shortcode += ' /]';
            } else {
                shortcode += ']';
		...
		toggleTransparentHeader: function() {
			...
			if ( 'undefined' !== typeof FusionApp && 'off' === FusionApp.preferencesData.transparent_header ) {
				...
				$body.removeClass( 'fusion-no-absolute-containers' );
			}
		},
		...
		/**
		 * Toggles device rendering logic preview.
		 *
		 * @since 3.13.3
		 * @return {void}
		 */
		toggleDeviceRenderingLogicPreview: function() {
			const $elements = jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).find( '.awb-device-rendering-logic' );

			$elements.each( function() {
				const cid       = jQuery( this ).hasClass( 'fusion-fullwidth' ) ? jQuery( this ).parent().data( 'cid' ) : jQuery( this ).data( 'cid' ),
		...
		/**
		 * Toggles inline dynamic data preview.
		 *
		 * @since 3.13.0
		 * @return {void}
		 */
		toggleInlineDynamicDataPreview: function() {
			if ( 'undefined' !== typeof FusionApp && 'never' === FusionApp.preferencesData.inline_dynamic_data_preview ) {
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).addClass( 'awb-inline-dynamic-data-preview-never ' );
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).removeClass( 'awb-inline-dynamic-data-preview-editing' ).removeClass( 'awb-inline-dynamic-data-preview-always' );
			} else if ( 'undefined' !== typeof FusionApp && 'editing' === FusionApp.preferencesData.inline_dynamic_data_preview ) {
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).addClass( 'awb-inline-dynamic-data-preview-editing' );
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).removeClass( 'awb-inline-dynamic-data-preview-never' ).removeClass( 'awb-inline-dynamic-data-preview-always' );
			} else {
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).addClass( 'awb-inline-dynamic-data-preview-always' );
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).removeClass( 'awb-inline-dynamic-data-preview-never' ).removeClass( 'awb-inline-dynamic-data-preview-editing' );
			}
		},
