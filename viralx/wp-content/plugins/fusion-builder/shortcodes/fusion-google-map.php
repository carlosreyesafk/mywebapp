		...
		class FusionSC_GoogleMap extends Fusion_Element {
			...
			public static function get_element_extras() {
				...
				return [
					...
					'embed_types'      => $fusion_settings->get( 'privacy_embed_types' ),
				];
			}
			...
			public function use_js_api() {
				...
				$overlay_color            = Fusion_Color::new_color( $this->args['overlay_color'] );
				...
				if ( $address ) {
					...
					if ( 'theme' === $map_style ) {
						...
						$animation                = 'yes';
						$infobox                  = 'custom';
						$overlay_color            = Fusion_Color::new_color( $fusion_settings->get( 'primary_color' ) );
						$infobox_background_color = FusionBuilder::hex2rgb( $overlay_color->color );
						$infobox_background_color = 'rgba(' . $infobox_background_color[0] . ', ' . $infobox_background_color[1] . ', ' . $infobox_background_color[2] . ', 0.8)';
						$brightness_level         = $overlay_color->brightness;
						...
						if ( $overlay_color->is_color_transparent() ) {
							$overlay_color = '';
						}
					}
					...
					foreach ( $this->args['address'] as $key => $address ) {
						$json_addresses[] = [
							'address'         => $address,
							'infobox_content' => html_entity_decode( esc_js( $this->args['infobox_content'][ $key ] ) ),
						];
					...
					$this->map_id      = $map_id;
					$overlay_color_hsl = [
						'hue' => $overlay_color->hue,
						'sat' => $overlay_color->saturation,
						'lum' => $overlay_color->lightness,
					];

					$overlay_color = ( ! $this->args['overlay_color'] || ! $overlay_color ) ? '' : $overlay_color->color;
					...
					<script type="text/javascript">
						...
						function fusion_run_map_<?php echo esc_html( $map_id ); ?>() {
							jQuery ('#<?php echo esc_html( $map_id ); ?>' ).fusion_maps( {
								...
								infobox_background_color: '<?php echo esc_html( $infobox_background_color ); ?>',
								infobox_styling: '<?php echo esc_html( $infobox ); ?>',
								infobox_text_color: '<?php echo esc_html( $this->args['infobox_text_color'] ); ?>',
								map_style: '<?php echo esc_html( $map_style ); ?>',
								map_type: '<?php echo esc_html( $this->args['type'] ); ?>',
								marker_icon: '<?php echo esc_html( $icon ); ?>',
								overlay_color: '<?php echo esc_html( $overlay_color ); ?>',
								...
								zoom: <?php echo esc_html( $this->args['zoom'] ); ?>,
