...
if ( fusion_is_element_enabled( 'fusion_login' ) ||
	...
	if ( ! class_exists( 'FusionSC_Login' ) ) {
		...
		class FusionSC_Login extends Fusion_Element {
			...
			/**
			 * Whether the pw reveal script has already been added to the page or not.
			 *
			 * @static
			 * @access private
			 * @since 3.13.0
			 * @var bool
			 */
			private static $pw_reveal_added = false;			
			...
			public function render_login( $args, $content = '' ) {
				...
				if ( ! is_user_logged_in() ) {
					...
					$password = '<input type="password" name="pwd"' . $placeholder . ' value="" size="20" class="fusion-login-password input-text" id="user_pass-' . $this->login_counter . '" />';

					if ( 'yes' === $this->args['reveal_password'] ) {
						$html .= '<div class="awb-user-pw-reveal">';
							$html .= $password;
							$html .= '<i class="awb-user-pw-reveal-icon awb-icon-eye-slash" id="pwd_' . $this->counter . '"></i>';
						$html       .= '</div>';
					} else {
						$html .= $password;
					}
					...
					if ( 'yes' === $this->args['reveal_password'] ) {
						$html .= $this->get_pw_reveal_script();
					}
			...
			/**
			 * Handles reveal password click.
			 *
			 * @since 3.13.0
			 *
			 * @return string The script tag.
			 */
			public function get_pw_reveal_script() {
				if ( self::$pw_reveal_added ) {
					return;
				}

				self::$pw_reveal_added = true;				
				ob_start();
				?>
				<script type="text/javascript">
					jQuery( document ).ready( function() {
						
						jQuery( '.awb-user-pw-reveal-icon' ).on( 'click', function( event ) {
			...
			[
				...
				'params'      => [
					...
					[
						...
						'value'       => '',
						'default'     => $fusion_settings->get( 'h3_typography', 'color' ),
						'dependency'  => [
							[
								'element'  => 'heading',
								'value'    => '',
					...
					[
						'type'        => 'radio_button_set',
						'heading'     => esc_attr__( 'Show Password Reveal Icon', 'fusion-builder' ),
						'description' => esc_attr__( 'Controls if the password reveal icon should be displayed in the login form.', 'fusion-builder' ),
